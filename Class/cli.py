...
class CLI(rqlite):
    ...
    def addMachine(self,data):
        ...
        mount = data[5] if len(data) > 6 else "none"
        ports = data[4] if len(data) > 5 else 0
        response = self.execute(['INSERT INTO machines(name,os,cores,memory,deploy,ports,mount) VALUES(?, ?, ?, ?, ?, ?, ?)',data[0],data[1],data[2],data[3],data[4],ports,mount])
