...
class CLI(rqlite):
    ...
    def addMachine(self,data):
        ...
        response = self.execute(['INSERT INTO machines(name,replica,os,cores,memory,storage,deploy,ports,mount) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)',data[0],data[1].replace("r",""),data[2],data[3],data[4].replace("MB",""),data[5].replace("GB",""),data[6],ports,mount])
