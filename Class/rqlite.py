...
class rqlite:
    ...
    def init(self):
        ...
        self.execute(["CREATE TABLE haproxy (machine TEXT NOT NULL, domain TEXT NOT NULL PRIMARY KEY, FOREIGN KEY(machine) REFERENCES machines(name) ON DELETE CASCADE)"])
        self.execute(["CREATE TABLE network (machine TEXT NOT NULL, node TEXT NOT NULL, ipv4 TEXT, FOREIGN KEY(machine) REFERENCES machines(name) ON DELETE CASCADE)"])
